...
| sortDirections | Supported sort way, could be `ascend`, `descend`. Sorts are cycled through in the order of the array when the sort button is clicked. This applies to all columns when set on the table props. You can disable the sort reset to default state by setting it like `['ascend', 'descend', 'ascend']` | Array | \[`ascend`, `descend`] | 3.0 |
